...
export default function PasswordForm({ getPassword }) {
  const { lengthValues, charCt, passParams, setIsChecked, clearForm } =
  ...
  return (
    <form className='PasswordForm flex col' onSubmit={handleSubmit}>
      <FaCog className='abso' size={30} color='#889696' onClick={toggleModal} />
      <Modal
      ...
      {opts}
      <button type='submit' disabled={passParams.length ? false : true}>
        Generate
      </button>
    </form>
  );
